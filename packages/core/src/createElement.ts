...
export class JSXNode {
  ...
  public upsertJSXChildren = (children: AsJSXChildren<RawChild>) => {
    derive(() => {
      const childrenNode = new Set(prepareChildrenNodes(children));
      this.removeExtraNodes(childrenNode);
      childrenNode.forEach((child) => {
        this.appendChildJSXNode(child);
      });
    });
  };
...
const JSXFactory = () => {
  ...
  const createElement = <P extends object>(
    ...
    children != null && jsxNode.upsertJSXChildren(children);
