...
pub fn transform_expression_with_tracker(expr: &AnyJsExpression, tracker: &mut HelperUsageTracker) -> Option<AnyJsExpression> {
    if let AnyJsExpression::JsxTagExpression(jsx_tag) = expr {
        if let Ok(jsx_element_any) = jsx_tag.tag() {
            ...
            } else if let AnyJsxTag::JsxSelfClosingElement(self_closing) = jsx_element_any {
                tracker.create_element = true;
                if let Some(expr) = crate::transformations::jsx_template::create_solidjs_call_with_tracker_self_closing(&self_closing, tracker) {
                    return Some(expr);
                }
            } else if let AnyJsxTag::JsxFragment(fragment) = jsx_element_any {
        ...
        return None;
    } else if let AnyJsExpression::JsArrowFunctionExpression(arrow_fn) = expr {
        return transform_arrow_function(arrow_fn);
    } else {
        return None;
    }
}